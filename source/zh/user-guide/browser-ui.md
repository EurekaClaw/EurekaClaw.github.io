...
| 门控 | 出现时间 | 您能做什么 |
|---|---|---|
| **文献门控（Survey）** | 文献调研未找到任何论文 | 提供论文 ID 或 arXiv ID 重试，或在没有论文的情况下继续 |
| **方向门控（Direction）** | 构思阶段未返回任何候选研究方向 | 输入自定义方向，或直接接受原始猜想 |
| **理论审核门控（Theory Review）** | 定理证明器完成后 | 批准以继续写作阶段，或标记具体引理并说明原因——标记后理论智能体注入反馈重新运行；达到 `THEORY_REVIEW_MAX_RETRIES` 次重试（默认 3 次）后自动通过 |
